					 ...
					 const struct iiod_client_ops *ops)
{
	...
	printf("SAI: Setting timeout to %u ms\n", params->timeout_ms);
...
{
	printf("SAI: Setting timeout. Input timeout: %u\n", timeout);
	unsigned int remote_timeout = calculate_remote_timeout(timeout);
	printf("SAI: Calculated remote timeout: %u\n", remote_timeout);
	struct iiod_io *io;
	int ret;
	...
	if (iiod_client_uses_binary_interface(client)) {
		...
		printf("SAI: Responder timeout set to: %u\n", timeout);
					  ...
					  const char *src, size_t len)
{
	...
	printf("SAI: Executing command with dev: %u, code: %u, length: %lu\n", cmd.dev, cmd.code, length);
						...
						unsigned int nb_ctx_attrs)
{
	...
	if (!iiod_client_uses_binary_interface(client)) { //SAIDO: Changed
